                ...
                for key, value in body.items():
                    if key not in ('__module__', '__new__'):
                        if hasattr(CLASS, key):
                            setattr(CLASS,
                                    "_super__%s" % key, getattr(CLASS, key))
...
try:
    ...
    def get_jvm_path():
        def jvm_exists(jvm):
            return jvm is not None and os.path.exists(jvm)
            ...
            for jvm_root in known_locations:
                if os.path.exists(jvm_root):
                    ...
                    for folder in os.listdir(jvm_root):
                        # Look for libjvm.so
                        if jvm_exists(jvm_root +
                                      "/" + folder +
                                      "/jre/lib/i386/client/libjvm.so"):
                            return jvm

        if not jvm_exists(jvm):
            if os.getenv("JAVA_HOME", None) is not None:
                raise IOError("Unable to find a java runtime to use.\
                    JAVA_HOME is set to '%s',\
                    but I could not find a JVM to use there."
                              % os.getenv("JAVA_HOME"))
            else:
                raise IOError("Unable to find a java runtime to use. \
                        Please set JAVA_HOME to point to the folder \
                        that contains your jre or jdk.")
        return jvm
    ...
    try:
        ...
        raise Exception("Unable to start JVM, even though I found the \
                JVM path. If you are using windows, this may be due to \
                missing system DLL files, please see the windows \
                installation instructions in the neo4j documentation.", e)
    ...
    if isinstance(GraphDatabaseService, jpype.JPackage):
        raise ImportError("Cannot find Neo4j java classes, \
                used classpath: %s" % classpath)
    ...
    def java_type(obj):
        ...
        for pyType, jType in conversions:
            if isinstance(obj, pyType):
                return jType
        raise TypeError("I don't know how to convert this to a \
                java primitive: %s" % repr(obj))
    ...
    def to_java(value):
        if isinstance(value, dict):
            ...
            if len(value) == 0:
                raise TypeError('Empty lists are not supported, \
                        since we cannot determine what type of \
                        list they should be in Java.')
            pyList = value
